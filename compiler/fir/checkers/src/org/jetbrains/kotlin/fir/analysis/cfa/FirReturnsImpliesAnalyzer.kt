...
object FirReturnsImpliesAnalyzer : FirControlFlowChecker() {
    ...
    override fun analyze(graph: ControlFlowGraph, reporter: DiagnosticReporter) {
        ...
        val dataFlowGraph = graphRef.dataFlowGraph
        if (function !is FirContractDescriptionOwner || dataFlowGraph == null) return
        ...
        effects.forEach { effectDeclaration ->
            val wrongCondition = graph.exitNode.previousCfgNodes.any {
                isWrongConditionOnNode(it, effectDeclaration, function, logicSystem, dataFlowGraph)
            }
    ...
    private fun isWrongConditionOnNode(
        ...
        val flow = dataFlowGraph.flowOnNodes.getValue(node) as PersistentFlow
        ...
        if (effect.value != ConeConstantReference.WILDCARD) {
            ...
            if (resultExpression is FirConstExpression<*>) {
                ...
                val resultVar = dataFlowGraph.variableStorage.getOrCreateVariable(flow, resultExpression)
        ...
        val conditionStatements =
            effectDeclaration.condition.buildTypeStatements(function, logicSystem, dataFlowGraph.variableStorage, flow) ?: return false
